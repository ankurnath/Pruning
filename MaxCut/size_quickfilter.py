...
def quickfilter(dataset,max_budget,min_budget,delta,eps,eta):
    ...
    assert len(solution_unpruned) <= max_budget, 'Solution from  ground set exceeds the budget'
    ...
    assert len(solution_pruned) <= max_budget, 'Solution from pruned ground set exceeds the budget'
