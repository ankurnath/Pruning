...
def train(dataset,budgets,m,delta):
    ...
    # print(sorted([graph.degree(node) for node in graph.nodes()])[::-1][:20])

    for idx,budget in enumerate(budgets):

        solution = imm(graph=graph,seed_size=budget)
        # sprint([graph.degree(node) for node in solution])

        r[idx] = max(r[idx],max(ranks[node] for node in solution))
