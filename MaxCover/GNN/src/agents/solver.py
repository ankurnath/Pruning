    ...
    def step(self):
        ...
        Returns:
            reward (float): The reward recieved.
                ...
                the action is taken.
        """
        rewards_avaialable = self.env.merginal_gain
...
#         self.current_observation = torch.FloatTensor(self.current_observation).to(self.device)
        self.current_observation = self.current_observation.to(self.device)
...
#         self.current_observation = torch.FloatTensor(self.current_observation).to(self.device)
    ...
    def step(self):
        ...
        if action is not None:
            observation, reward, done, _ = self.env.step(action)
            self.current_observation = observation.to(self.device)
